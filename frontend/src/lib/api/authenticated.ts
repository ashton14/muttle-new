...
const createExercise = (api: AxiosInstance) => (
  data: Exercise
): Promise<SavedExercise> => api.post('exercises', data).then(res => res.data);

const updateExercise = (api: AxiosInstance) => (
  exerciseId: number,
  data: SavedExercise
): Promise<SavedExercise> =>
  api.put(`exercises/${exerciseId}`, data).then(res => res.data);

const getExercise = (api: AxiosInstance) => (
  exerciseId: number
): Promise<SavedExercise> =>
...
const getTestCases = (api: AxiosInstance) => (
  exerciseId: number,
  userId: number,
  actual = false
...
const createTestCase = (api: AxiosInstance) => (
...
const deleteTestCase = (api: AxiosInstance) => (
...
const runTests = (api: AxiosInstance) => (
  exerciseId: number,
  userId: number
): Promise<AttemptFeedback> =>
  api.post(`run/${exerciseId}`, {userId}).then(res => res.data);

const getLatestAttempt = (api: AxiosInstance) => (
  exerciseId: number,
  userId: number
): Promise<AttemptFeedback> =>
  api
